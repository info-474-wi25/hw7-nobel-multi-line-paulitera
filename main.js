...
d3.csv("nobel_laureates.csv").then(data => {
    ...
    const categories = d3.rollup(categorizedData,
        ...
        d => d.categoryGroup // group by the category group
    );
    ...
    const maxCount = d3.max(yearCounts, v => d3.max(v));

    // console.log("year flat map: ", allYears);
    // console.log("counts regular map: ", yearCounts);
